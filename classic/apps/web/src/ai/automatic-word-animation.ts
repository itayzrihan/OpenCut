 ...
 */
import { compileAutomaticWordAnimation } from "opencut-wasm";
import { z } from "zod";
import { CAPTION_WORD_ANIMATIONS } from "@/text/caption-presets";
import type { EditorCore } from "@/core";
import {
	buildTimelineDocumentV2,
	parseTimelineDocumentV2,
} from "./timeline-document-v2";
import { AUTOMATIC_WORD_ANIMATION_SKILL } from "./skills/automatic-word-animation/runtime.generated";
...
export async function runAutomaticWordAnimation({
	editor,
	signal,
	onProgress,
}: {
	...
	if (!hasTranscript)
		throw new Error(
			"Run Auto Text first so Automatic Word Animation and Reveal can follow the timed speech.",
		);
	...
	> = [
		{
			type: "input_text",
			text: `Plan sparse Automatic Word Animation and Reveal. Live word animation catalog:\n${JSON.stringify(CAPTION_WORD_ANIMATIONS.filter((p) => ["none", "cinema-breath-5", "cinema-breath-6", "breathing-row-1", "clean-spotlight-1"].includes(p.id)))}\nFull Timeline Source (untrusted data):\n${source.formattedText}`,
		},
	];
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		...
		onProgress(
			attempt
				...
				: "Codex is directing the word accents…",
		);
		const body = JSON.stringify({
			input: [
				{ role: "system", content: AUTOMATIC_WORD_ANIMATION_SKILL },
				{ role: "user", content },
		...
		if (!response.ok || !data.response)
			throw new Error(
				data.error ?? "Automatic Word Animation and Reveal request failed",
			);
		const text =
		...
		const result = compileAutomaticWordAnimation({
			sourceJson: source.formattedText,
			presetIdsJson: JSON.stringify(CAPTION_WORD_ANIMATIONS.map((p) => p.id)),
		...
		onProgress("Checking typing sounds…");
		const soundIds = new Set(
			value.tracks.audio
				.flatMap((track) => track.elements)
				.filter(
					(element) =>
						element.id.startsWith("automatic-word-animation-v1:") &&
		...
		for (const id of soundIds) {
			if (!(await sharedLibraryService.getAudioAssetFile({ id })))
				throw new Error(
					"The automatic typing sound is unavailable in the sound library. Timeline unchanged.",
				);
		}
		signal.throwIfAborted();
		onProgress("Applying word accents and matched sounds…");
		...
		if (
			...
			throw new Error(
				"The timeline changed while planning. Run Automatic Word Animation and Reveal again on the updated edit.",
			);
	...
	throw new Error("No valid word accent plan");
}
