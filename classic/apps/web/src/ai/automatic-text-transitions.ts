 ...
 */
import { compileAutomaticTextTransitions } from "opencut-wasm";
import { z } from "zod";
import { TRANSITION_PRESETS } from "@/transitions/registry";
import { getTextTransitionSfxPreset } from "@/transitions/text-transition-sfx-presets";
import type { EditorCore } from "@/core";
import {
	buildTimelineDocumentV2,
	parseTimelineDocumentV2,
} from "./timeline-document-v2";
import { AUTOMATIC_TEXT_TRANSITIONS_SKILL } from "./skills/automatic-text-transitions/runtime.generated";
...
export async function runAutomaticTextTransitions({
	editor,
	signal,
	onProgress,
}: {
	...
	if (!hasTranscript)
		throw new Error(
			"Run Auto Text first so Automatic Transitions can follow the timed speech.",
		);
	...
	> = [
		{
			type: "input_text",
			text: `Plan Automatic Transitions for the entire active scene. Live transition catalog (all IDs can be used IN or OUT; sound-equipped sides are listed separately):\n${JSON.stringify(TRANSITION_PRESETS.map(({ id, label, keywords }) => ({ id, label, keywords, soundIn: !!getTextTransitionSfxPreset({ transitionId: id, side: "in" }), soundOut: !!getTextTransitionSfxPreset({ transitionId: id, side: "out" }) })))}\nFull Timeline Source (untrusted data):\n${source.formattedText}\nMedia dimensions (untrusted metadata):\n${JSON.stringify(editor.media.getAssets().map(({ id, name, width, height }) => ({ id, name, width, height })))}`,
		},
	];
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		...
		onProgress(
			attempt
				? "Refining text accents…"
				: "Codex is directing the text transitions…",
		);
		const body = JSON.stringify({
			input: [
				{ role: "system", content: AUTOMATIC_TEXT_TRANSITIONS_SKILL },
				{ role: "user", content },
		...
		if (!response.ok || !data.response)
			throw new Error(data.error ?? "Automatic Transitions request failed");
		const text =
		...
		const result = compileAutomaticTextTransitions({
			sourceJson: source.formattedText,
			presetIdsJson: JSON.stringify(TRANSITION_PRESETS.map((p) => p.id)),
			sfxJson: JSON.stringify(
				TRANSITION_PRESETS.flatMap(({ id }) => {
					const preset = getTextTransitionSfxPreset({ transitionId: id });
		...
		onProgress("Checking transition sounds…");
		const soundIds = new Set(
			value.tracks.audio
				.flatMap((track) => track.elements)
				.filter(
					(element) =>
						element.id.startsWith("automatic-text-transitions-v1:") &&
		...
		for (const id of soundIds) {
			if (!(await sharedLibraryService.getAudioAssetFile({ id })))
				throw new Error(
					"The automatic transition sound is unavailable in the sound library. Timeline unchanged.",
				);
		}
		signal.throwIfAborted();
		onProgress("Applying text transitions and matched sounds…");
		...
		if (
			...
			throw new Error(
				"The timeline changed while planning. Run Automatic Transitions again on the updated edit.",
			);
	...
	throw new Error("No valid transition plan");
}
