...
| Unified Angles virtual multicam                                        | Classic UI/preview/export supports 2+ angles and cyclic multi-cut assignment; typed `media.angles.unify`, `timeline.item.angle.set`, and `timeline.items.angles.cycle` contracts live in `crates/editor-api`; rewrite UI migration pending |
